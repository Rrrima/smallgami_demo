...
export const MODELS = [
  ...
  { id: 'gemini-2.5-flash', label: 'Nano Banana (2.5 Flash)', provider: 'gemini' },
...
const DEFAULT_INFO = { name: '', characteristics: '', speed: 50, mass: 50, stability: 50, jumpPower: 50 }
...
function defaultInfoForStyle(styleName) {
  const map = {
    'Pixel Art': { name: 'Blip McSquare', characteristics: 'Born in 1987, still thinks 64 colors is plenty.', speed: 60, mass: 40, stability: 70, jumpPower: 55 },
    'Sketch': { name: 'Doodle McDraw', characteristics: 'Escaped from a napkin and never looked back.', speed: 50, mass: 45, stability: 55, jumpPower: 50 },
    'Ghibli': { name: 'Whisper Cloudwing', characteristics: 'Fluent in wind and can nap on a moving cloud.', speed: 45, mass: 55, stability: 60, jumpPower: 65 },
  }
...
function buildPrompt(style, hasSketch, provider) {
  if (provider === 'openai') {
    if (hasSketch) {
      const p = `Here is a user's hand-drawn sketch of a bird. You MUST preserve the bird's unique shape, proportions, features, colors, and personality from the sketch — this is the user's original creation and their design choices must be respected. Restyle it in ${style.shortDesc}, but keep the same silhouette and distinctive characteristics. CRITICAL: The bird MUST face RIGHT (facing to the right side of the image), side view profile. If the sketch faces left, MIRROR/FLIP it so it faces RIGHT. Fully TRANSPARENT background (no background at all). Game sprite asset. No text, no watermarks, no shadows, no ground.`
      return p
    }
    const p = `CRITICAL: The bird MUST face RIGHT (facing to the right side of the image), side view profile. A cute colorful bird, on a fully TRANSPARENT background (no background at all, just the bird). Game sprite asset with transparency, ${style.shortDesc}. No text, no watermarks, no shadows, no ground.`
    return p
  }

  const basePrompt = `You are a game asset designer. Generate ONLY a single bird character sprite for a flappy bird game.
...
function buildRefinedPrompt() {
  return `Clean up and smooth the lines of this sketch. Keep the exact same design, colors, shape, and proportions. Do not add creative elements or change the style. Just refine and polish the drawing.
...
export async function generateWorldAssets(birdImageDataUrl, modelId = 'gemini-2.0-flash') {
...
No characters, no obstacles, no pipes, no tubes, no UI, no text. Output ONLY the image.`
  ...
  if (model.provider === 'openai') {
    ...
    skyboxPromise = callGemini(model.id, [imageData, { text: skyboxPrompt }]).then(r => r.image)
    pipePromise = callGemini(model.id, [imageData, { text: pipePrompt }]).then(r => r.image)
  }
...
export async function generateBirdVariations(sketchDataUrl, modelId = 'gemini-2.0-flash') {
  const model = MODELS.find(m => m.id === modelId) || MODELS[0]
  ...
  if (hasSketch) {
    ...
    if (model.provider === 'openai') {
      refinedPromise = callOpenAI(refinedPrompt, sketchDataUrl).then(image => ({
        image,
        info: { name: 'Glow-Up', characteristics: 'Same bird, but after a spa day and a pep talk.', speed: 50, mass: 50, stability: 50, jumpPower: 50 },
        label: 'Refined',
        _provider: 'openai',
      }))
    } else {
