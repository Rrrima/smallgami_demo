...
export const PROMPTS = {
  ...
  birdBase: (styleDesc) => `You are a game asset designer. Generate ONLY a single bird character sprite for a flappy bird game.
...
Output ONLY the image. No text, no labels, no watermarks.`,
  ...
  openai: {
    birdFromSketch: `Here is a user's hand-drawn sketch of a bird. Refer to the user's original creation. CRITICAL: The bird MUST face RIGHT, side view. If the sketch faces left, MIRROR/FLIP it so it faces RIGHT. This is a player asset for a cute game. Fully TRANSPARENT background (no background at all). Game sprite asset. No text, no watermarks, no shadows, no ground.`,

    birdGenerate: (shortStyleDesc) => `CRITICAL: The bird MUST face RIGHT (facing to the right side of the image), side view profile. A cute colorful bird, on a fully transparent background. Game sprite asset with transparency, ${shortStyleDesc}. No text, no watermarks, no shadows, no ground.`,

    skybox: `This is a bird character. Generate ONLY a game background/skybox that matches this bird's art style and theme. Seamless horizontally tileable landscape — just pure environment art (sky, clouds, landscape, horizon). Do NOT include any pipes, tubes, pillars, obstacles, characters, or game UI. No characters, no obstacles, no pipes, no UI, no text.`,

    pipe: `This is a bird character. Generate a single vertical pipe/pillar game obstacle asset that matches this bird's art style and theme. Transparent background, this is a game asset. The pipe should be oriented vertically, centered in the frame. No characters, no text.`,
  },

  gemini: {
    skybox: `Here is a bird character for a flappy bird game. Generate ONLY a game background/skybox that matches this bird's art style and theme.
