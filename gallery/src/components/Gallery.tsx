...
interface GLink extends SimulationLinkDatum<GNode> {
  ...
  gradient?:   boolean;  // fade opacity from source to target
}
...
// Color for primitives that span two categories — no type hub in the graph.
const COMBINED_COLOR = '#c084fc';
    ...
    <>
      ...
      {show && pos && createPortal(
        <span
          ...
          style={{ top: pos.top - 8, right: pos.right - 4 }}
        >
          {text}
        </span>,
        document.body,
...
function GridView({ onSelect }: Props) {
  ...
  const renderedGames = games.filter(g => !g.primitive);

  const handleEnter = (game: GameMeta) => {
    if (game.primitive) setHoveredPrimId(game.config.id);
    else if (game.primitiveId) setHoveredPrimId(game.primitiveId);
  };
  const handleLeave = () => setHoveredPrimId(null);

  const primClass = (game: GameMeta) => {
    if (!hoveredPrimId) return '';
    return hoveredPrimId === game.config.id ? ' is-highlighted' : ' is-dimmed';
  };

  const renderedClass = (game: GameMeta) => {
    if (!hoveredPrimId) return '';
    return game.primitiveId === hoveredPrimId ? ' is-highlighted' : ' is-dimmed';
  };
  ...
  return (
    <div className="grid-view">
      ...
      <div className="grid-section">
        <div className="grid-section-label">Primitives</div>
        <div className="grid-flat grid-flat--sm">
          {primGames.map(game => {
            ...
            return (
              <div
                key={game.config.id}
                className={`grid-card grid-card--sm is-primitive${primClass(game)}`}
                data-morph-id={game.config.id}
                style={{ '--c': color, '--c-border': color + '44' } as React.CSSProperties}
                onClick={() => onSelect(game)}
                onMouseEnter={() => handleEnter(game)}
                onMouseLeave={handleLeave}
              >
                <span
                  className="card-tag"
                  style={{ '--c': color, '--c-border': color + '55' } as React.CSSProperties}
                >
                  {label}
                </span>
                ...
                {game.primitiveInfo && <InfoTooltip text={game.primitiveInfo} />}
              </div>
            );
          })}
        </div>
      ...
      {/* Rendered games */}
      <div className="grid-section">
        <div className="grid-section-label">Games</div>
        <div className="grid-flat">
          {renderedGames.map(game => {
            ...
            return (
              <div
                key={game.config.id}
                className={`grid-card${renderedClass(game)}`}
                data-morph-id={game.config.id}
                style={{ '--c': color, '--c-border': color + '44' } as React.CSSProperties}
                onClick={() => onSelect(game)}
                onMouseEnter={() => handleEnter(game)}
...
function GraphView({ onSelect }: Props) {
  ...
  useEffect(() => {
    ...
    // Combined-color primitives bridge two existing categories; they get no
    // type hub of their own and are excluded from centroid calculations.
    ...
    for (const g of primGames) {
      ...
      if (color === COMBINED_COLOR) continue; // no type hub for combined primitives
      const label = clusterLabel(color);
    ...
    // Root → Type  (structural backbone — gradient fade from root outward)
    ...
    // Combined-color primitives link to BOTH Dodge and Catcher type nodes.
    for (const n of allNodes.filter(n => n.nodeType === 'primitive')) {
      if (n.color === COMBINED_COLOR) {
        links.push({ source: 'type-#f59e0b', target: n.uid, color: '#f59e0b', opacity: 0.75, dashed: true });
        links.push({ source: 'type-#06b6d4', target: n.uid, color: '#06b6d4', opacity: 0.75, dashed: true });
      } else {
        links.push({
          source:  `type-${n.color}`,
          target:  n.uid,
      ...
      .force('link',
        forceLink<GNode, GLink>(links)
          ...
          .distance((d) => {
            ...
            if (s.nodeType === 'root' || t.nodeType === 'root') return 90;
            if (s.nodeType === 'type' || t.nodeType === 'type') return 120;
            return 155;
          })
          .strength((d) => {
            const s = d.source as GNode;
            const t = d.target as GNode;
            if (s.nodeType === 'root' || t.nodeType === 'root') return 1.2;
            return 0.7;
          }))
      .force('charge',
        forceManyBody<GNode>()
          .strength((d: GNode) => d.nodeType === 'root' ? -600 : d.nodeType === 'type' ? -500 : -380)
          .distanceMin(20)
          .distanceMax(600))
      .force('collide',
        forceCollide<GNode>((d: GNode) => d.nodeType === 'root' ? 56 : d.nodeType === 'type' ? 52 : 52)
          .strength(0.9))
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    if ((e.target as HTMLElement).closest('.info-trigger')) return;
          ...
          <defs>
            {linksRef.current.map((lnk, i) => {
              ...
              return (
                ...
                >
                  <stop offset="0%" stopColor={lnk.color} stopOpacity={0.0} />
                  <stop offset="50%" stopColor={lnk.color} stopOpacity={0.25} />
                  <stop offset="100%" stopColor={lnk.color} stopOpacity={0.6} />
                </linearGradient>
              );
            })}
          </defs>
          {linksRef.current.map((lnk, i) => {
            ...
            return (
              <g key={i}>
                <line
                  x1={s.x} y1={s.y} x2={t.x} y2={t.y}
                  stroke={lnk.gradient ? `url(#edge-grad-${i})` : lnk.color}
                  strokeOpacity={lnk.gradient ? (linked ? 1 : 0.04) : linkOpacity}
          ...
          .map(n => {
            ...
            return (
              ...
              >
                ...
                {n.game?.primitiveInfo && <InfoTooltip text={n.game.primitiveInfo} />}
              </div>
            );
          })}
